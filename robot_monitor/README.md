...
- **Live map + robot position** -- base grid on `/limo/map` (transient-local), incremental patches on `/limo/map_updates` (`map_msgs/OccupancyGridUpdate`, merged locally instead of re-requesting the whole grid each time) rendered as a grayscale image (free=white, occupied=black, unknown=gray). A second **Contour** layer works the same way on `/limo/contour_map` + `/limo/contour_map_updates` -- toggle between them with the Map/Contour buttons above the map. Robot position + heading (blue circle + red line) comes from **TF** (`map` -> `limo/base_footprint`), the exact frames used by your existing `robot_pose_gui/limo_pose_gui.py` on the robot -- reused directly rather than guessed, including its quaternion-to-yaw math. Polled once per spin tick (50 Hz) rather than a separate thread, since everything already runs off one `QTimer`.
...
├── map_utils.py      -- OccupancyGrid/OccupancyGridUpdate -> QImage (base grid + merged patches) + world<->pixel transform
...
- Whether `/limo/map` and `/limo/contour_map` are actually published transient-local on your setup (if a map panel stays blank, try re-publishing/relaunching your map source, or drop the `DurabilityPolicy.TRANSIENT_LOCAL` in `ros_bridge.py` to match whatever QoS it's actually using).
- Whether `/limo/map_updates` / `/limo/contour_map_updates` use the standard `map_msgs/OccupancyGridUpdate` message shape (`x`, `y`, `width`, `height`, `data` as a sub-rectangle patch) -- that's what `OccupancyGridState.apply_update()` in `map_utils.py` assumes. If updates never seem to land, this is the first thing to check.
- `map_msgs` needs to be available in your ROS2 environment (it's a standard package, normally installed alongside the navigation stack).
